...
total = 0
numbers = [*"0123456789"]
numbersSpelled = ["THISWONTHAPPEN", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
...
for line in dayOneInput.split("\n"):
    #firstNum = None
    #lastNum = None
    
    #numArr = []
    numArr = [None] * len(line)
    
    print(line)
    
    for spelled in numbersSpelled:
        try:
            res = [i for i in range(len(line)) if line.startswith(spelled, i)]
            for occure in res:
                numArr[occure] = numbers[numbersSpelled.index(spelled)]
                #numArr.insert(occure, numbers[numbersSpelled.index(spelled)])
            print(res)
            #j = line.index(spelled)
            #numArr.insert(j, numbers[numbersSpelled.index(spelled)])
            #print(line, j, spelled, )
        except ValueError: continue
    
    for num in numbers:
        try:
            res = [i for i in range(len(line)) if line.startswith(num, i)]
            for occure in res:
                numArr[occure] = numbers[numbers.index(num)]
            print(res)
        except ValueError: continue
    
    """
    for char in line:        
        try: i = numbers.index(char)
        except ValueError: continue

        i = line.index(char)
        numArr[i] = char
        #numArr.insert(i, char)
        print(i, char, len(line))
        #numArr[i] = char
        #if firstNum == None: firstNum = char
        #lastNum = char
    """
    
    numArr = remove_items(numArr, None)
    
    print(numArr)
    
    numTogether = int(f"{numArr[0]}{numArr[-1]}")
    #numTogether = int(f"{firstNum}{lastNum}")
    print(f"{numArr[0]} {numArr[-1]}", numTogether)
    total += numTogether
